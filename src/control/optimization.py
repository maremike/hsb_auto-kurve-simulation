...
def ineq_constraints(x):
    """
    Defines inequality constraints for the optimization process based on input parameters. Uses the inaccuracy tolerance
    value to speed up the optimization and allow slight inperfections.

    Constraints:
        0: |f_static_friction| = |f_neutral| * staticFriction
        1: f_centripetal = f_gravity_parallel + f_static_friction
        2: f_road = -f_neutral
        3: |f_velocity| = |f_new_velocity|
        4: f_velocity = -f_drag
        5: f_velocity = f_new_velocity + f_centrifugal
        6: f_gravity = f_neutral + f_gravity_parallel
        7: f_centripetal = -f_centrifugal
        8: f_new_velocity = transform_vector(transform_vector(f_velocity, 0, turnAngle, 0), 0, 0, turnIncline) -> angles in rad
        9: turnIncline = arctan(f_gravity_parallel[1] / f_gravity_parallel[0]) -> (arctan(y/x) = arctan(m))
    ...
    # |f_static_friction| = |f_neutral| * staticFriction
    ...
    # f_new_velocity = transform_vector(transform_vector(f_velocity, 0, turnAngle, 0), 0, 0, turnIncline)
